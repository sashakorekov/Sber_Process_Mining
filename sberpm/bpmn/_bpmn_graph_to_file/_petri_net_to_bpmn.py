...
from ...visual._graph import Node


def petri_net_to_bpmn(petri_net):
    """
    ...
    # Remove unneeded edges for parallel gateways and maybe the gateways.
    # Possible situation:
    # start_event -> excl_gateway_1 -> task_1 -> parallel_gateway_1 -> task_2 -> endevent
    # start_event -> excl_gateway_1-> parallel_gateway_1 -> task_2 -> endevent
    # In this case the edge "excl_gateway_1-> parallel_gateway_1" is incorrect and unneeded.
    for gateway_node in gateway_nodes:
        if gateway_node.type == NodeType.PARALLEL_GATEWAY:
            gnode = graph.nodes[gateway_node.id]
            modified = False
            for edge in gnode.input_edges:
                if edge.source_node.id == NodeType.START_EVENT:
                    graph.remove_edge_by_src_trg_id(edge.source_node.id, edge.target_node.id)
                    modified = True
                    break
            if not modified:
                for edge in gnode.output_edges:
                    if edge.target_node.id == NodeType.END_EVENT:
                        graph.remove_edge_by_src_trg_id(edge.source_node.id, edge.target_node.id)
                        modified = True
                        break
            if modified:
                if len(gnode.input_edges) == len(gnode.output_edges) == 1:
                    graph.remove_node_by_id(gnode.id)  # source an target nodes will be reconnected

    # Create a gateway after the startevent if needed
    node = graph.nodes[NodeType.START_EVENT]
    target_nodes = [edge.target_node for edge in node.output_edges]
    if len(target_nodes) > 1:
        gateway_node, gateway_node_number = create_gateway_node(node.type, gateway_node_number)
        insert_node(graph, gateway_node, [node], target_nodes)

    # Create a gateway before the endevent if needed
    node = graph.nodes[NodeType.END_EVENT]
    source_nodes = [edge.source_node for edge in node.input_edges]
