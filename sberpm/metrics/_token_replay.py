...
class TokenReplay:
    ...
    def apply(self):
        ...
        c_list = np.zeros((unique_id_number,), dtype=int)
        p_list = np.zeros((unique_id_number,), dtype=int)
        m_list = np.zeros((unique_id_number,), dtype=int)
        r_list = np.zeros((unique_id_number,), dtype=int)
        ...
        df = pd.DataFrame({self._data_holder.id_column: self._data_holder.grouped_data[self._data_holder.id_column],
                           self._data_holder.activity_column: traces,
                           'c': c_list, 'p': p_list, 'm': m_list, 'r': r_list,
    ...
    def _calc_coeffs(trace, replay_graph):
        ...
        for act in trace:
            ...
            if len(node.incoming) == 0 or len(node.outgoing) == 0:  # Ignore separate (disconnected) nodes
                continue
            else:
...
class ReplayTransition(ReplayAbstractNode):
    def __init__(self):
        """
        Representation of a transition that is suitable for token replay.
        """
        super().__init__()
